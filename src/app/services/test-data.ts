...
type ExpenseTemplate = { category: string, description: string, cost: number, day: number };
...
const parisExpenses: ExpenseTemplate[] = [
  { category: '🏨', description: 'Hôtel des Grands Boulevards (3 nights)', cost: 96000, day: 30 },
  { category: '🚂', description: 'Thalys Amsterdam–Paris return', cost: 38400, day: 30 },
  { category: '🎟️', description: 'Louvre tickets', cost: 3600, day: 29 },
  { category: '🍽️', description: 'Dinner at Brasserie Lipp', cost: 18600, day: 29 },
  { category: '🥐', description: 'Poilâne bakery', cost: 1480, day: 28 },
  { category: '🎟️', description: 'Eiffel Tower summit', cost: 5800, day: 28 },
  { category: '🚂', description: 'Paris Visite metro pass (3 days)', cost: 5200, day: 27 },
  { category: '🛒', description: 'Monoprix groceries', cost: 3640, day: 27 },
  { category: '🍽️', description: 'Sunday lunch at Le Comptoir du Relais', cost: 9200, day: 26 },
  { category: '🎟️', description: 'Centre Pompidou', cost: 4400, day: 26 },
  { category: '🥐', description: 'Breakfast at Café de Flore', cost: 5600, day: 25 },
  { category: '🛒', description: 'Marché d\'Aligre picnic supplies', cost: 2880, day: 25 },
  { category: '🍽️', description: 'Tasting menu at Septime', cost: 34000, day: 24 },
  { category: '🎟️', description: 'Musée d\'Orsay', cost: 4200, day: 24 },
  { category: '🚗', description: 'Uber from CDG airport', cost: 6800, day: 23 },
  { category: '🧴', description: 'Pharmacie Monge toiletries', cost: 4200, day: 23 },
  { category: '🍽️', description: 'Angelina hot chocolate & mont-blanc', cost: 3800, day: 22 },
  { category: '🎟️', description: 'Comédie-Française theatre', cost: 11600, day: 22 },
  { category: '🛒', description: 'La Grande Épicerie du Bon Marché', cost: 8400, day: 21 },
  { category: '🍽️', description: 'Dinner at Chez Georges', cost: 12400, day: 21 },
  { category: '🥐', description: 'Pierre Hermé macarons', cost: 2960, day: 20 },
  { category: '🚂', description: 'RER to Versailles', cost: 1480, day: 20 },
  { category: '🎟️', description: 'Château de Versailles day pass', cost: 5800, day: 19 },
  { category: '🧽', description: 'Pressing du Marais dry cleaning', cost: 2800, day: 19 },
  { category: '🍽️', description: 'Farewell dinner at Le Grand Véfour', cost: 46000, day: 18 },
];
...
const spainExpenses: ExpenseTemplate[] = [
  ...
  { category: '🥐', description: 'Horno de San Buenaventura', cost: 1140, day: 20 },
  { category: '🎟️', description: 'Real Alcázar de Sevilla', cost: 2800, day: 20 },
  { category: '🍽️', description: 'Tapas at Bar El Rinconcillo', cost: 9600, day: 19 },
...
export class TestDataService {
  ...
  async generate(): Promise<void> {
    let user1 = await this.userStore.save({ name: 'Christopher', email: 'christopher@example.com', friends: [] });
    let user2 = await this.userStore.save({ name: 'Nathaniel', email: 'nathaniel@example.com', friends: [] });
    let user3 = await this.userStore.save({ name: 'Samantha', email: 'samantha@example.com', friends: [] });
    ...
    const group1 = await this.groupStore.save({ name: 'Bloemendaal aan Zee 2025', users: [user1.id, user2.id], createdBy: user1.id });
    const group2 = await this.groupStore.save({ name: 'Paris long weekend', users: [user2.id, user3.id], createdBy: user2.id });
    const group3 = await this.groupStore.save({ name: 'Costa del Sol summer', users: [user3.id, user1.id], createdBy: user3.id });
  ...
  private async addExpenses(
    ...
    userA: EntityId,
    userB: EntityId,
    expenses: { category: string, description: string, cost: number, day: number }[],
  ): Promise<void> {
    ...
    for (let i = 0; i < expenses.length; i++) {
      ...
      await this.expenseStore.save({
        groupId, createdBy: paidBy(i), currency: '€',
        category, description, cost,
        date: daysAgo(day), shares: split(cost),
      });
    }
  }
